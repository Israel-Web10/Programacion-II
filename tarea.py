...
class TeatroMunicipalApp:
    ...
    def mostrar_boletos(self):
        ...
        scroll.pack(side=tk.RIGHT, fill=tk.Y)  # Y en mayúscula

        texto = tk.Text(frame, yscrollcommand=scroll.set, wrap=tk.WORD)  # WORD en lugar de NORD
        texto.pack(fill=tk.BOTH, expand=True)

        scroll.config(command=texto.yview)  # yview correctamente escrito
